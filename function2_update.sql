...
BEGIN
    ...
    END LOOP;
     ...
     -- вычисление хеша
    FOR rec IN SELECT * FROM temp_accounts_data LOOP
        record_hash := md5(
            rec.accounts_fk::TEXT || rec.case_id_fk::TEXT || rec.case_card_number || 
            rec.case_date_open::TEXT || rec.case_date_close::TEXT || 
            rec.case_medical_organization::TEXT || rec.case_department::TEXT || 
            rec.patient_id::TEXT || COALESCE(rec.patient_age::TEXT, '') || 
            COALESCE(rec.patient_full_name, '') || COALESCE(rec.patient_birth_date::TEXT, '') || 
            COALESCE(rec.document_id::TEXT, '') || COALESCE(rec.document_type_code, '') || 
            COALESCE(rec.document_series, '') || COALESCE(rec.document_number, '') ||
            rec.is_filtered::TEXT || COALESCE(rec.filter_reason, '')
        );
        
        UPDATE temp_accounts_data
        SET hash = record_hash
        WHERE accounts_fk = rec.accounts_fk AND case_id_fk = rec.case_id_fk;
    END LOOP;
